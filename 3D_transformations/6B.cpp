...
float ptsIni[8][3] = {{40, 40, -50}, {90, 40, -50}, {90, 90, -50},
...
void matrixSetIdentity(Matrix4x4 m)  // Initialises the matrix as Unit Matrix
{
    int i, j;
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++) m[i][j] = (i == j);
}

void matrixPreMultiply(
    Matrix4x4 a,
    ...
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++) theMatrix[i][j] = tmp[i][j];
}
...
void display(void) {
    ...
    switch (choiceRot) {
        ...
        case 4:
            DrawRotLine();
            float MOD =
                sqrt((x2 - x1) * (x2 - x1) + (y2 - y11) * (y2 - y11) +
                        (z2 - z1) * (z2 - z1));
            aa = (x2 - x1) / MOD;
            bb = (y2 - y11) / MOD;
            cc = (z2 - z1) / MOD;
