...
class matrix {
   public:
    ...
    matrix(int s) {
        n = s;
        val.resize(n);
        for (int i = 0; i < n; i++) val[i].resize(n);
    }
...
namespace Geometry {
vector<vector<float>> a = {{40, 40, -50}, {90, 40, -50}, {90, 90, -50},
                           {40, 90, -50}, {30, 30, 0},   {80, 30, 0},
                           {80, 80, 0},   {30, 80, 0}};
vector<vector<float>> colors = {{0.7, 0.4, 0.7}, {0.2, 0.5, 0.3},
                                {0.2, 0.4, 0.7}, {0.5, 0.4, 0.3},
                                {0.5, 0.7, 0.2}, {0.2, 0.3, 0.4}};
...
void Main() {
    ...
    auto transalte = [&](float x, float y, float z) {
        matrix<float> mat(4);
        mat.val = {{1, 0, 0, x}, {0, 1, 0, y}, {0, 0, 1, z}, {0, 0, 0, 1}};
    ...
    auto rotation = [&](float theta) {
        matrix<float> mat(4);
        mat.val = {{cos(theta), -sin(theta), 0, 0},
...
int main(int argc, char **argv) {
    ...
    glOrtho(-300, 300.0, -300.0, 300.0, -300.0, 300.0);
